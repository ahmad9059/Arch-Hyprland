...
whiptail --title "HyprFlux Arch-Hyprland (2025) Install Script" \
  --msgbox "Welcome to HyprFlux Arch-Hyprland (2025) Install Script!!!\n\n\
...
while true; do
  ...
  if [[ "$dots_selected" == "OFF" ]]; then
    ...
    if ! whiptail --title "HyprFlux Hyprland Dot Files" --yesno \
      "You have not selected to install the pre-configured HyprFlux Hyprland dotfiles.\n\nKindly NOTE that if you proceed without Dots, Hyprland will start with default vanilla Hyprland configuration and I won't be able to give you support.\n\nWould you like to continue install without HyprFlux Hyprland Dots or return to choices/options?" \
  ...
  echo "👌 ${OK} You confirmed your choices. Proceeding with ${SKY_BLUE}HyprFlux 🇵🇰 Hyprland Installation...${RESET}" | tee -a "$LOG"
  break
done
